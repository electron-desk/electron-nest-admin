 ...
 * @LastEditTime: 2022-09-02 22:00:20
 * @LastEditors: 最爱白菜吖
 * @Description:
...
@ApiTags('管理员')
@ApiBearerAuth()
@ApiExtraModels(Admin, LoginResponseDto, CurrentResponseDto, CaptchaDto)
@Controller('admin')
export class AdminController {
  ...
  async login(@Body() loginDto: LoginDto) {
    const captcha = await this.cacheManager.get<string>(loginDto.cacheKey);
    if (!captcha || captcha.toLowerCase() !== loginDto.captcha.toLowerCase()) {
      return error('验证码错误');
    }
  ...
  async captcha() {
    ...
    const cacheKey = randomStr(16);
    await this.cacheManager.set(cacheKey, text, 300 * 1000);
    return success({
      captcha:
        'data:image/svg+xml;base64, ' + Buffer.from(captcha).toString('base64'),
      cacheKey,
    });
  }
  @ApiOperation({
